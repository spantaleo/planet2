...
class MDSliderRenderCss{
  ...
  public function getCSS($index = array(), $layer) {
    ...
    if ($css != "") {
      if (isset($layer['link']) && is_array($layer['link'])) {
        $classes[] = ".md-layer-{$index[0]}-{$index[1]}-{$index[2]}{font-size: inherit; }";
        $classes[] = ".md-layer-{$index[0]}-{$index[1]}-{$index[2]} a" . $css;
      }
      else {
        $classes[] = ".md-layer-{$index[0]}-{$index[1]}-{$index[2]}" . $css;
      }
    }

    // Generate css for hover link
    if (isset($layer['link']) && is_array($layer['link'])) {
      $classes[] = ".md-layer-{$index[0]}-{$index[1]}-{$index[2]} {z-index: {$layer["zindex"]}!important;}";
      $css = $this->generateLayerLinkCss($layer['link']);
      if ($css != '') {
        if ($layer['type'] != "text") {
          $classes[] = ".md-layer-{$index[0]}-{$index[1]}-{$index[2]} a:hover img" . $css;
        }
        else {
          $classes[] = ".md-layer-{$index[0]}-{$index[1]}-{$index[2]} a:hover" . $css;
        }
      }
    }
